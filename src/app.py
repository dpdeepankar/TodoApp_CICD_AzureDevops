...
# Initialize database
def init_db():
    #Call psycopg2.connect() to create a connection to the database todo.db in the current working directory, creating it if it does not exist.
    try: 
        with psycopg2.connect(f"host={POSTGRES_HOST} user={POSTGRES_USER} password={POSTGRES_PASSWORD} dbname=postgres") as conn:
            ...
            with conn.cursor() as cur:
                ...
                try:
                    cur.execute("SELECT 1 FROM pg_database WHERE datname='todoapp'")
                    if not cur.fetchone():
                        cur.execute("CREATE DATABASE todoapp")
        ...
        with psycopg2.connect(f"host={POSTGRES_HOST} user={POSTGRES_USER} password={POSTGRES_PASSWORD} dbname=todoapp") as conn:
            with conn.cursor() as cur:
...
def adduser():
    ...
    conn = psycopg2.connect(f"host={POSTGRES_HOST} user={POSTGRES_USER} password={POSTGRES_PASSWORD} dbname=todoapp")
    cur = conn.cursor()

    try:
        # Check if the email is already registered.
        res = cur.execute('SELECT 1 FROM users WHERE EMAIL = %s',(email,))
        user = cur.fetchone()
        
        # If the user is not registered already, then add details from the registration form to DB and redirect to login
        if not user:
            cur.execute('INSERT INTO users (EMAIL, FULL_NAME, PASSWORD) VALUES (%s,%s,%s);',(email,fullname,passwd))
            conn.commit()
...
def loginuser():
    ...
    if 'email' in session:
        ...
        try:
            ...
            cur.execute('SELECT TASKS,STATUS FROM tasks WHERE EMAIL = %s', (user_email,))
            tasks = cur.fetchall()
        ...
        try:
            # Fetch user info
            cur.execute('SELECT PASSWORD, FULL_NAME FROM users WHERE EMAIL = %s', (email,))
            result = cur.fetchone()
            user_passwd = result[0]
            fullname = result[1]

            if result is None:
                flash("User does not exist. Please register to continue.")
                return render_template('userLogin.html')
            ...
            # Fetch user tasks 
            try:
                cur.execute('SELECT TASKS,STATUS FROM tasks WHERE EMAIL = %s', (email,))
                tasks = cur.fetchall()
                print(tasks)
            except Exception as e:
                print(f"Task retrieval error: {e}")
...
def addtask():
    ...
    try:
        cur.execute('INSERT INTO tasks(TASKS, EMAIL, STATUS) VALUES (%s,%s,%s)',(task, email, status))
...
def completetask():
    ...
    try:
        cur.execute('UPDATE tasks SET STATUS = %s WHERE TASKS = %s AND EMAIL = %s',(status, task, email))
...
def deletetask():
    ...
    try:
        cur.execute('DELETE FROM tasks WHERE TASKS = %s AND EMAIL = %s',( task, email))
        flash("Task deleted!! ")
    except Exception as e:
        print(f"Error: {e}")
    finally:
